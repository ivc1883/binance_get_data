...
def all_ids():
    ...
    if exchange_found:
        ...
        for (k, v) in tuples:
            if (re.search('.BTC', v['id']) or re.search('.USDT', v['id'])): 
                r=requests.get('https://api.binance.com/api/v1/klines' + '?symbol=' + str(v['id']) + '&interval=' + '1h'+ '&limit=' + str(1))
...
def give_first_kline_open_stamp(interval, symbol, start_ts):
    ...
    url_to_get = "https://api.binance.com/api/v1/klines?symbol="+ str(symbol) + "&interval="+ str(interval) + "&startTime="+ str(start_ts) 
...
for this_id in ids:
    ...
    '''
    timeframe = '1h' #Available values: '1m', '3m', '5m', '15m', '30m', '1h', '2h', '4h', '6h', '8h', '12h', '1d', '3d', '1w', '1M'
    '''**********************************************************************************************************************************************
       **********************************************************************************************************************************************
    '''
    ...
    try:
      xx = open('BIN_'+this_id+'_'+timeframe+'.json')
      print('Existing Data')
      with open('BIN_'+this_id+'_'+timeframe+'.json', 'r') as json_file :
    ...
    for _ in range(0,int(requireds)):
      ...
      url = "https://api.binance.com/api/v1/klines?symbol=" + str(this_id) + "&interval=" + str(timeframe) + "&startTime=" + str(start_date) + "&endTime=" + str(int(end_date)) + "&limit=" + str(1000)  # 1000 is becuase of the limit, max 1000 
    ...
    with open('BIN_'+this_id+'_'+timeframe+'.json','w') as f:
      f.write(str(final_data))

    
    with open('BIN_'+this_id+'_'+timeframe+'.json', 'r') as f :
      filedata = f.read()
